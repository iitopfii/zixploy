...
### Fixed
- **นำเข้า container แล้วได้ตัวแปรที่ไม่ได้ตั้งเองติดมาด้วย** — ค่าที่ image กำหนดมาเอง (เช่น
  เวอร์ชันของ nginx) ถูกนับเป็น environment variable ของผู้ใช้ ทำให้แยกไม่ออกว่าอะไรคือค่าที่
  ตั้งไว้จริง · ตอนนี้เหลือเฉพาะที่ตั้งเองตอนสร้าง container (ถ้าตั้งทับค่าของ image ก็ยังเก็บให้)
...
### Added
- **นำเข้า container ที่มีอยู่แล้วบนเซิร์ฟเวอร์** — container ที่สร้างไว้เองก่อนหน้า (หรือมาจาก
  docker compose ของตัวเอง) เดิมเห็นได้แค่ในหน้า Docker แต่จัดการอะไรไม่ได้ · ตอนนี้กด "นำเข้า"
  ที่รายการนั้น ระบบจะอ่านค่าตั้งค่าของ container (image, คำสั่ง, port, volume, ชื่อ environment
  variable) มาแสดงให้ตรวจก่อน แล้วสร้างเป็น project ให้เมื่อยืนยัน
  - **ไม่แตะ container เดิมเลย** — ของเดิมทำงานต่อตามปกติ และระบบจะยังไม่ deploy จนกว่าจะกดเอง
  - ค่าของ environment variable ไม่ถูกแสดงบนหน้าเว็บและไม่ถูกเก็บแบบอ่านได้ — ระบบอ่านจาก
    container แล้วเข้ารหัสให้ตอนยืนยันเท่านั้น
  - volume ที่ container เดิมใช้ยังไม่ถูกผูกให้อัตโนมัติ (เพิ่มได้ที่แท็บ Volumes ของ project)
...
### Changed
- **ชื่อ container อ่านออกแล้วว่าเป็นของ project ไหน** — เดิมเห็นแต่รหัสยาว ๆ
  (`zx-01kzcx…-01kzve…`) ต้องเปิด dashboard เทียบทุกครั้ง · ตอนนี้ขึ้นต้นด้วยชื่อ project เช่น
  `giveme-zx-…` และโปรเจกต์แบบหลาย container มีชื่อส่วนประกอบด้วย (`giveme-web-zx-…`) ทำให้
  `docker ps` บนเซิร์ฟเวอร์อ่านรู้เรื่องทันที
  - รหัสยังต่อท้ายครบเหมือนเดิมและยังเป็นตัวชี้ขาด การเปลี่ยนชื่อ project จึงไม่กระทบ container
    ที่รันอยู่ · ชื่อที่ใช้อักขระที่ Docker ไม่รองรับ (เช่นภาษาไทยล้วน) จะได้ชื่อรูปแบบเดิม
  - container ที่รันอยู่ตอนนี้ยังชื่อเดิม จะเปลี่ยนเป็นชื่อใหม่เองตอน deploy ครั้งถัดไป
...
### Fixed
- **ปุ่ม "ตรวจ DNS" กดแล้วเหมือนไม่มีอะไรเกิดขึ้น** — ถ้าเรียกตรวจไม่สำเร็จ (เช่น session หมดอายุ
  หรือเข้าผ่าน host ที่ระบบยังไม่รู้จัก) หน้าเว็บจะเงียบไปเฉย ๆ แยกไม่ออกว่า "ตรวจแล้วยังไม่เจอ
  DNS" หรือ "เรียกตรวจไม่สำเร็จ" · ตอนนี้แสดงสาเหตุจากเซิร์ฟเวอร์ใต้สถานะของ domain นั้น
...
### Fixed
- **ติดตั้งด้วย domain แล้วเว็บเข้าไม่ได้เลย (ต่อจาก 0.1.17)** — การเปิด HTTPS ในรุ่นก่อนหน้า
  ประกาศเส้นทางภายในซ้ำกับที่มีอยู่แล้ว ทำให้ Traefik ปฏิเสธการจัดเส้นทางของทั้ง dashboard และ
  API พร้อมกัน ผลคือเข้าไม่ได้ทั้ง http และ https (ขึ้น `404 page not found`) · แก้ให้ใช้เส้นทาง
  เดิมที่มีอยู่แล้ว — ใครที่ติดตั้ง/อัปเดตเป็น 0.1.17 พร้อม domain ควรอัปเดตรุ่นนี้ทันที
...
### Security
- **เปิด HTTPS ให้หน้า dashboard และ API เมื่อติดตั้งด้วย domain** — เดิมตัวจัดการระบบทำงานบน
  HTTP อย่างเดียวทุกการติดตั้ง รหัสผ่านและ session จึงเดินทางแบบไม่เข้ารหัสทุกครั้งที่เข้าใช้งาน
  (ทั้งที่เว็บของผู้ใช้ได้ HTTPS อัตโนมัติอยู่แล้ว) · ติดตั้งด้วย `ZIXPLOY_DOMAIN=...` ตอนนี้ได้
  ใบรับรองจาก Let's Encrypt อัตโนมัติ พร้อมพา http ไป https ให้เอง — ติดตั้งด้วย IP ทำงานเหมือนเดิม
  แต่จะมีคำเตือนบอกว่ายังไม่ได้เข้ารหัสและวิธีเปลี่ยนไปใช้ domain

### Fixed
- **ตั้ง domain ตอนติดตั้งแล้ว login ไม่ผ่าน** — ระบบตั้งค่าให้เข้าผ่าน https แต่ยังไม่มีเส้นทาง
  https จริงให้เข้า ทำให้เข้าไม่ได้ทั้งสองทาง แก้พร้อมกับการเปิด HTTPS ด้านบน
- **หน้า Deploy ไม่รีเฟรชเอง ต้องกดเองตลอด** — เดิมหน้าจะอัปเดตอัตโนมัติเฉพาะตอนที่มี deploy
  ทำงานอยู่แล้วเท่านั้น เปิดหน้าทิ้งไว้แล้วมี deploy ใหม่เริ่มจากที่อื่น (เช่น push เข้า repo)
  จึงไม่เห็นจนกว่าจะกดรีเฟรช · ตอนนี้อัปเดตให้ต่อเนื่อง ถี่ขึ้นตอนมีงานกำลังทำ และหยุดยิงข้อมูล
  เมื่อสลับไปแท็บอื่น แล้วรีเฟรชทันทีที่กลับมา
- **worker ขึ้น error ทุกครั้งที่อัปเดตพร้อมโครงสร้างฐานข้อมูลใหม่** — worker กับ API เริ่มพร้อมกัน
  worker จึงเห็นฐานข้อมูลรุ่นเก่าอยู่ครู่หนึ่งเป็นเรื่องปกติ แต่เดิมมันจะหยุดตัวเองทันทีแล้วรอ
  restart ทำให้มี error ค้างใน log ทุกครั้งที่อัปเดต · ตอนนี้รอจนพร้อมแล้วทำงานต่อ (ถ้ารอนานเกินไป
  ยังหยุดพร้อมแจ้งเหตุผลเหมือนเดิม)
...
### Fixed
- **ปุ่มอัปเดตไม่เคยอัปเดตไฟล์ `docker-compose.yml`** — ตัวอัปเดตเดิมแค่ดึง image ใหม่แล้ว
  restart โดยใช้ไฟล์ compose ตัวเดิมที่วางไว้ตอนติดตั้งครั้งแรก การเปลี่ยนแปลงโครงสร้างระบบใน
  รุ่นถัดมา (service ใหม่, ตัวแปรที่เพิ่มเข้ามา, การตั้งค่า proxy) จึงไปไม่ถึงเครื่องที่อัปเดต
  ผ่านหน้าเว็บเลย มีแต่คนติดตั้งใหม่เท่านั้นที่ได้ · ตอนนี้ตัวอัปเดตดึงไฟล์ของรุ่นเป้าหมายมาใช้
  ด้วย โดยตรวจความถูกต้องก่อนใช้เสมอ และถ้าอัปเดตล้มเหลวจะคืนไฟล์เดิมแล้วพยุงระบบกลับขึ้นมาทันที
  (ดึงไฟล์ไม่ได้ก็ใช้ของเดิมต่อ ไม่ทำให้การอัปเดตล้มทั้งรุ่น)
