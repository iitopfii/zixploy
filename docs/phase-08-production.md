...
- [x] M4: Backup automation (`bun run backup` → `internal/db/scripts/backup.ts`) — SQLite snapshot + master key file + Traefik ACME storage แยก directory กัน (`ZIXPLOY_BACKUP_{DB,KEYS,ACME}_DIR`), retention ลบไฟล์เก่าอัตโนมัติ (`ZIXPLOY_BACKUP_RETENTION`, default 14); GitHub App private key/webhook secret ครอบคลุมด้วย DB backup อยู่แล้ว (เป็น ciphertext ใน DB); runbook เต็มที่ `docs/runbooks/backup-restore.md`
- [x] M5: Web security — secure headers/CSRF/login rate limit มีมาตั้งแต่ Phase 1 (`plugins/security-headers.ts`, `plugins/auth.ts` assertCsrf, `auth/rate-limit.ts`); เพิ่มใน M5: HSTS header (production only, `security-headers.ts`), session rotation ทุก `SESSION_ROTATION_INTERVAL_MS` (24h) แบบโปร่งใส (`auth/session.ts`: `needsRotation`, wired ใน `plugins/auth.ts`), Origin/Host validation (`plugins/origin-guard.ts`) — allowlist มาจาก host ของ `ZIXPLOY_BASE_URL`; ไม่ได้ตั้งค่าใน production = ข้ามการตรวจไปเลยพร้อม warning log (fail open เพื่อไม่บล็อกทั้งระบบ) ต้องตั้งค่าเองก่อน deploy จริง, อนุโลม localhost ใน dev/test
- [x] M6: Production docker-compose (`deploy/control-plane/docker-compose.prod.yml`) — 4 services (traefik, control-api, deploy-worker, dashboard) พร้อม resource limits (128M–2G memory, 0.5–2.0 cpu), ACME production TLS, healthchecks, separated networks; `.env.production.example`; runbooks ครบ: `control-plane-down`, `deployment-stuck`, `docker-daemon-unavailable`, `disk-full`, `certificate-failed`, `github-app-revoked`, `rotate-github-credentials`, `release-checklist`; `CHANGELOG.md`
